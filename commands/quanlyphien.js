...
class QuanLyPhienCommand extends Command {
  ...
  registerApplicationCommands(registry) {
    registry.registerChatInputCommand(builder =>
      builder.setName('quanlyphien').setDescription('📋 Quản lý phiên điểm danh — xem, kết thúc, hủy, lịch sử')
        .addStringOption(opt => opt.setName('che_do').setDescription('Chế độ xem').setRequired(false)
          .addChoices({ name: '🟢 Phiên đang chạy', value: 'active' }, { name: '📚 Lịch sử phiên', value: 'history' }))
    );
  }
  ...
  async chatInputRun(interaction) {
    ...
    if (mode === 'active') {
      ...
      collector.on('collect', async (btn) => {
        ...
        if (btn.customId.startsWith('qp_end_') || btn.customId.startsWith('qp_cancel_')) {
          ...
          const confirmEmbed = new EmbedBuilder().setColor(0xa13544).setTitle(`⚠️ Xác nhận ${label} phiên?`).setDescription(`Bạn có chắc muốn **${label}** phiên \`${sid.slice(0, 8)}\` không?\nHành động này **không thể hoàn tác**.`);
          return btn.editReply({ embeds: [confirmEmbed], components: [confirmRow(action, sid)] });
        }
        if (btn.customId.startsWith('qp_confirm_')) {
          const parts = btn.customId.split('_'), realAction = parts[2], realSid = parts.slice(3).join('_');
          if (realAction === 'end')    await db.endSession(realSid);
          if (realAction === 'cancel') await db.cancelSession(realSid);
          const doneEmbed = new EmbedBuilder().setColor(realAction === 'end' ? 0x437a22 : 0xa12c7b)
            .setTitle(realAction === 'end' ? '✅ Phiên đã kết thúc' : '🚫 Phiên đã bị hủy')
            .setDescription(`Phiên \`${realSid.slice(0, 8)}\` đã được ${realAction === 'end' ? 'kết thúc' : 'hủy'} thành công.`).setTimestamp();
    ...
    if (mode === 'history') {
      ...
      const msg = await interaction.editReply({ embeds: [buildHistoryListEmbed(sessions)], components });
      if (!sessions.length) return;

      const collector = msg.createMessageComponentCollector({ time: 5 * 60_000, filter: i => i.user.id === interaction.user.id });
      collector.on('collect', async (comp) => {
        await comp.deferUpdate();
        if (comp.customId === 'qp_history_select') {
          const sid = comp.values[0], s = await db.getSessionByIdRaw(sid, guildId);
          if (!s) return comp.editReply({ content: '⚠️ Không tìm thấy phiên.', embeds: [], components: [] });
          const attendances = await db.getAttendances(sid);
          return comp.editReply({ embeds: [buildSessionDetailEmbed(s, attendances)], components: [backRow()] });
        }
        if (comp.customId === 'qp_back_history') {
          const ss2 = await db.getSessionHistory(guildId, { limit: 20, offset: 0 });
          return comp.editReply({ embeds: [buildHistoryListEmbed(ss2)], components: ss2.length ? [historySelectRow(ss2)] : [] });
        }
      });
      collector.on('end', (_, reason) => { if (reason === 'time') interaction.editReply({ components: [] }).catch(() => {}); });
    }
