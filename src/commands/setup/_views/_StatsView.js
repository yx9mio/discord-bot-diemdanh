...
function renderToi(stats, member, guild, badges) {
  ...
  const badgeStr = (badges ?? []).length
    ? badges.map(b => { const t = BADGE_THRESHOLDS.find(bt => bt.threshold === b.threshold); return t ? `${t.emoji} **${t.label}**` : `🎖️ ${b.threshold}`; }).join('  ')
    : '_Chưa có huy hiệu_';
  const embed = new EmbedBuilder()
    .setColor(COLORS.GOLD)
    .setTitle(`${ICONS.PERSON} ${name}`)
    .setDescription([phong ? `> 📌 **${phong}**` : null, `${pctEmoji(pct)} **Tỉ lệ tham gia: ${pct}%** — ${pctLabel(pct)}`, `\`${bar}\``].filter(Boolean).join('\n'))
    .addFields(
  ...
  if (typeof member?.displayAvatarURL === 'function') { const url = member.displayAvatarURL({ dynamic: true, size: 64 }); if (url) embed.setThumbnail(url); }
  return { embeds: [embed], components: [] };
}

function renderRank(rows, guild, topN = 10) {
  if (!rows?.length) return { embeds: [new EmbedBuilder().setColor(COLORS.GOLD).setTitle(`${ICONS.TROPHY} Bảng xếp hạng`).setDescription('> _Chưa có dữ liệu._').setFooter({ text: FOOTER_DEFAULT }).setTimestamp()], components: [] };
  const medals = ['🥇', '🥈', '🥉'];
  const lines = rows.slice(0, topN).map((r, i) => {
    const medal = medals[i] ?? `\`${String(i + 1).padStart(2)}.\``;
    const name  = guild?.members?.cache?.get(r.user_id)?.displayName ?? `<@${r.user_id}>`;
    const joined = r.total_joined ?? 0; const streak = r.current_streak ?? 0;
    const total = r.total_sessions ?? joined; const pct = total > 0 ? Math.round(joined / total * 100) : 0;
    return `${medal} **${name}**\n\`${buildRichProgressBar(pct, 8)}\` ${pct}% · ${joined} phiên · ${ICONS.FIRE}${streak}`;
  });
  return { embeds: [new EmbedBuilder().setColor(COLORS.GOLD).setTitle(`${ICONS.TROPHY} Top ${Math.min(rows.length, topN)} — Bảng xếp hạng`).setDescription(lines.join('\n\n')).setFooter({ text: `${FOOTER_DEFAULT} · Cập nhật lần cuối` }).setTimestamp()], components: [] };
}

function renderLichSu(records, userId, guild, page = 0) {
  const PAGE_SIZE = 10; const total = records.length;
  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const clampedPage = Math.max(0, Math.min(page, totalPages - 1));
  const start = clampedPage * PAGE_SIZE; const slice = records.slice(start, start + PAGE_SIZE);
  const name = guild?.members?.cache?.get(userId)?.displayName ?? `<@${userId}>`;
  if (!total) return { embeds: [new EmbedBuilder().setColor(COLORS.PRIMARY).setTitle(`📋 Lịch sử — ${name}`).setDescription('> _Chưa có điểm danh nào._').setFooter({ text: FOOTER_DEFAULT }).setTimestamp()], components: [] };
  const statusMap = { tham_gia: { emoji: ICONS.ATTEND_YES }, tre: { emoji: ICONS.ATTEND_LATE }, khong_tham_gia: { emoji: ICONS.ATTEND_NO }, co_phep: { emoji: ICONS.ATTEND_EXCUSE } };
  const lines = slice.map((r, i) => {
    const s = statusMap[r.status] ?? { emoji: '❓' };
    const time = r.checked_in_at ? `<t:${Math.floor(new Date(r.checked_in_at).getTime() / 1000)}:d>` : '—';
    return `\`${String(start + i + 1).padStart(2)}.\` ${s.emoji} **${r.sessions?.session_name ?? 'Phiên'}** · ${time}`;
  });
  const embed = new EmbedBuilder().setColor(COLORS.PRIMARY).setTitle(`📋 Lịch sử — ${name}`).setDescription(lines.join('\n')).setFooter({ text: `${FOOTER_DEFAULT} · Trang ${clampedPage + 1}/${totalPages} · Tổng ${total} lần` }).setTimestamp();
  const navRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('setup:stats:lichsu:prev').setLabel('◄ Trang trước').setStyle(ButtonStyle.Secondary).setDisabled(clampedPage === 0),
    new ButtonBuilder().setCustomId('setup:stats:lichsu:next').setLabel('Trang sau ►').setStyle(ButtonStyle.Secondary).setDisabled(clampedPage >= totalPages - 1),
  );
  return { embeds: [embed], components: [navRow], _page: clampedPage, _totalPages: totalPages };
}

function renderServerStats(stats) {
  const pct = stats?.rate_present ?? 0; const total = stats?.total_sessions ?? 0;
  const members = stats?.total_members ?? 0; const attends = stats?.total_attendances ?? 0;
  const color = pct >= 80 ? COLORS.GREEN : pct >= 50 ? COLORS.YELLOW : COLORS.RED;
  const embed = new EmbedBuilder().setColor(color).setTitle(`${ICONS.CHART} Thống kê Server`)
    .setDescription([`${pctEmoji(pct)} **Tỉ lệ tham gia trung bình: ${pct}%** — ${pctLabel(pct)}`, `\`${buildRichProgressBar(pct)}\``].join('\n'))
    .addFields(
...
function renderXemInput() {
  const embed = new EmbedBuilder().setColor(COLORS.PRIMARY).setTitle('🔍 Xem điểm danh người khác').setDescription('Nhập **User ID** hoặc **@mention** vào modal.').setFooter({ text: FOOTER_DEFAULT }).setTimestamp();
  const navRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(CUSTOM_ID.REFRESH).setLabel('Làm mới').setEmoji(ICONS.REFRESH).setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(CUSTOM_ID.BACK_HOME).setLabel('← Bảng điều khiển').setEmoji(ICONS.HOME).setStyle(ButtonStyle.Secondary),
  );
...
module.exports = { StatsView: { renderStatsMenu, renderToi, renderRank, renderLichSu, renderServerStats, renderXemInput, CUSTOM_ID }, BADGE_THRESHOLDS };
