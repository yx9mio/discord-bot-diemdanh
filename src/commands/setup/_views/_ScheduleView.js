...
function render({ schedules, page = 0, guild }) {
  ...
  if (total > 0) {
    for (const s of slice)
      delRow.addComponents(new ButtonBuilder().setCustomId(`${CUSTOM_ID.DEL_PREFIX}${s.id}`).setLabel(`✕ ${DAY_VI[s.day_of_week]} ${String(s.hour).padStart(2,'0')}:${String(s.minute).padStart(2,'0')}`).setStyle(ButtonStyle.Danger));
  }

  const ctrlRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(CUSTOM_ID.ADD_R).setLabel('+ Thêm hằng tuần').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId(CUSTOM_ID.ADD_O).setLabel('+ Thêm một lần').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId(CUSTOM_ID.PAGE_PREV).setLabel('◀').setStyle(ButtonStyle.Secondary).setDisabled(clampedPage === 0),
    new ButtonBuilder().setCustomId(CUSTOM_ID.PAGE_NEXT).setLabel('▶').setStyle(ButtonStyle.Secondary).setDisabled(clampedPage >= totalPages - 1),
  );
