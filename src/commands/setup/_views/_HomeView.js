...
function render({ guild, cfg, schedules, members, session }) {
  ...
  const drilldown = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(CUSTOM_ID.CFG).setLabel('Cài đặt chung').setEmoji(ICONS.GEAR).setStyle(ButtonStyle.Secondary),
    ...
    new ButtonBuilder().setCustomId(CUSTOM_ID.STATS).setLabel('Thống kê').setEmoji('🏆').setStyle(ButtonStyle.Secondary),
  );
  ...
  const sessionBtns = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(CUSTOM_ID.SESSION).setLabel(session ? 'Đóng phiên' : 'Đóng phiên').setEmoji(ICONS.CLOSE).setStyle(session ? ButtonStyle.Danger : ButtonStyle.Secondary).setDisabled(!session),
    new ButtonBuilder().setCustomId(CUSTOM_ID.BROADCAST).setLabel('Phát tin').setEmoji('📢').setStyle(ButtonStyle.Secondary),
  );
