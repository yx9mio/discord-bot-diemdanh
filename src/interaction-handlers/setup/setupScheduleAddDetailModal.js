...
class SetupScheduleAddDetailModalHandler extends InteractionHandler {
  ...
  parse(interaction) {
    // [BUG-FIX] MODAL_RECURRING_ID không còn được generate (recurring dùng select
// flow setup:sch:add:r:*) — chỉ parse onetime để khỏi nhận interaction lạ
  ...
  async run(interaction) {
    ...
    try {
      ...
      // [BUG-FIX] Nhánh recurring modal bỏ (recurring dùng select flow setup:sch:add:r:*)
...
module.exports = { SetupScheduleAddDetailModalHandler, MODAL_ONETIME_ID };
