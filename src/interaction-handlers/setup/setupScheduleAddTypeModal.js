...
const CUSTOM_ID = Object.freeze({
  ...
  EDIT_BTN_PREFIX:'setup:sch:edit:',       // setup:sch:edit:<id>

  // Modal bước 1: chọn loại
  TYPE_MODAL:     'setup:sch:add:type',

  // Modal bước 2a: recurring weekly
  RECURRING_MODAL:'setup:sch:add:recurring:detail',

  // Modal bước 2b: one-time (gộp ngày + giờ)
  ONETIME_MODAL:  'setup:sch:add:onetime:detail',

  // Modal sửa one-time
  EDIT_ONETIME_PREFIX: 'setup:sch:edit:onetime:',

  // Pagination / điều hướng trong ScheduleView
...
class SetupScheduleAddTypeModalHandler extends InteractionHandler {
  ...
  async run(interaction) {
    ...
    if (loai === 'once' || loai === 'one_time' || loai === 'onetime' || loai === '1') {
      // [BUG-3] ONE-TIME: gộp ngày + giờ + phút vào 1 modal (tránh 2 lần showModal)
      const modal = new ModalBuilder()
...
module.exports = {
  SetupScheduleAddBtnHandler,
  SetupScheduleAddTypeModalHandler,
  CUSTOM_ID,
};
