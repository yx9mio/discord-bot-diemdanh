...
class SetupMemberAddHandler extends InteractionHandler {
  ...
  parse(interaction) {
    if (ADD_IDS.has(interaction.customId)) return this.some();
    return this.none();
  }
...
class SetupMemberAddModalHandler extends InteractionHandler {
  ...
  async run(interaction) {
    ...
    // Validate user tồn tại trong guild
    let member;
    try {
    ...
    if (member.user.bot) {
      return interaction.editReply({ content: '❌ Không thể thêm bot vào danh sách.' });
    }

    const username = displayName ?? member.nickname ?? member.user.displayName ?? member.user.username;

    try {
      await memberService.upsertMember(guild.id, {
        user_id: userId,
        username,
...
module.exports = { SetupMemberAddHandler, SetupMemberAddModalHandler };
