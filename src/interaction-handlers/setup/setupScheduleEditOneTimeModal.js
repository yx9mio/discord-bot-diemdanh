    ...
    const modal = new ModalBuilder()
      ...
      .addComponents(
        ...
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            .setCustomId('phut_bu')
            .setLabel('Phiên dài (phút, 0 = không tự đóng)')
            .setStyle(TextInputStyle.Short)
            .setValue(phutBuVal)
            .setRequired(false),
        ),
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            ...
            .setLabel('Nhắc nhở trước (phút, 0 = tắt)')
            .setStyle(TextInputStyle.Short)
            .setValue(preVal)
            .setRequired(false),
        ),
...
class SetupScheduleEditOneTimeModalHandler extends InteractionHandler {
  ...
  async run(interaction) {
    ...
    if (!gio) {
      return interaction.editReply({ content: '❌ Giờ mở không hợp lệ. Định dạng HH:MM.' });
    }
    ...
    try {
      ...
      log.info('SETUP_SCH_EDIT', guild.id, 'Sửa lịch one-time #%s → %s %02d:%02d +%dm',
