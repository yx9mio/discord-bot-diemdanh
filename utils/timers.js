...
async function datHenGioDong(client, guild, session, channelId, ms) {
  ...
  t.autoClose = setTimeout(async () => {
    // BUG-11 fix: xóa timer khỏi Map ngay khi bắt đầu chạy (dùng finally để
    // đảm bảo luôn được dọn dù bất kỳ bước nào throw)
    try {
      const cur = await db.getActiveSession(guild.id);
      if (!cur || cur.id !== session.id) return;
      ...
      try {
        ...
        // Không return — tiếp tục dọn dẹp dù DB fail
      }
      ...
      // BUG-11 fix: luôn xóa khỏi Map dù success hay throw
