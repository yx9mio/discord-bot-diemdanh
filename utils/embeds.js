...
async function buildDisplayEmbed(guild, session, ended = false) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(ended ? Colors.Grey : Colors.Gold).setTimestamp();
  const statsLines = [
  ...
  if (joined.length > 0) {
    ...
    chunkLines(lines, 950).forEach((chunk, i) => embed.addFields({ name: i===0 ? `✅ Tham Gia (${joined.length})` : '\u200b', value: chunk, inline: false }));
  }
  if (declined.length > 0) {
    const lines = declined.map((x, i) => `\`${String(i+1).padStart(2)}.\` ${x.name}`);
    chunkLines(lines, 950).forEach((chunk, i) => embed.addFields({ name: i===0 ? `❌ Không Tham Gia (${declined.length})` : '\u200b', value: chunk, inline: false }));
  }
  if (!ended && eligible > 0) {
    ...
    if (absentIds.length > 0) {
      ...
      const MAX_SHOW = 20;
      const names = absentIds.slice(0, MAX_SHOW).map(id => { const m = guild.members.cache.get(id); return m ? m.displayName : `<@${id}>`; });
...
function buildSummaryEmbed(session) {
  ...
  const pct = eligible > 0 ? Math.round((joined.length / eligible) * 100) : 0;
  const bar = buildProgressBar(pct);
  return new EmbedBuilder().setTitle(`📊 Tổng Kết: ${session.session_name}`).setColor(Colors.Green)
    .setDescription(`📈 Tỷ lệ: \`${bar}\` **${pct}%** (${joined.length}/${eligible})`)
    .addFields(
      { name: `✅ Tham Gia (${joined.length})`, value: joined.map((x,i) => `\`${String(i+1).padStart(2)}.\` ${x.name} (${x.time})`).join('\n').slice(0,1024) || '—', inline: false },
      { name: `❌ Không Tham Gia (${declined.length})`, value: declined.map((x,i) => `\`${String(i+1).padStart(2)}.\` ${x.name}`).join('\n').slice(0,1024) || '—', inline: false },
    ).setTimestamp();
}

function buildHistoryEmbed(history) {
  if (history.length === 0) return new EmbedBuilder().setTitle('📜 Lịch Sử Điểm Danh').setColor(Colors.Blue).setDescription('Chưa có phiên nào được ghi lại.');
  const lines = history.slice(-10).reverse().map((s, i) => {
  ...
  return new EmbedBuilder().setTitle(`📜 Lịch Sử Điểm Danh (${history.length} phiên)`).setColor(Colors.Blue).setDescription(lines.join('\n\n')).setTimestamp();
}

function buildStatsEmbed(title, lines) {
  return new EmbedBuilder().setTitle(title).setColor(Colors.Purple).setDescription(lines.length > 0 ? lines.join('\n') : 'Chưa có dữ liệu.').setTimestamp();
}

function buildConfigEmbed(cfg, attendanceRole, adminRole) {
  return new EmbedBuilder().setTitle('⚙️ Cấu Hình Bot Điểm Danh').setColor(Colors.Blurple)
    .addFields(
      { name: '🎯 Role Điểm Danh', value: attendanceRole ? `${attendanceRole} (${cfg.allowed_role_name})` : `❌ Chưa cài — tên mặc định: **${cfg.allowed_role_name}**`, inline: false },
      { name: '🛡️ Role Admin Bot', value: adminRole ? `${adminRole} (${cfg.admin_role_name})` : `❌ Chưa cài — tên mặc định: **${cfg.admin_role_name}**`, inline: false },
    ).setTimestamp();
}
