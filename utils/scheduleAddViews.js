...
function renderAddViewStep2(guild, state) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription([
      ...
      '',
      'Chọn **ngày kết thúc**, **giờ**, **phút** và **kênh** (tuỳ chọn), sau đó Xác nhận.',
    ].join('\n'))
    .setFooter({ text: FOOTER_DEFAULT })
    .setTimestamp();

  const closeDayOpts = CLOSE_DAY_OFFSETS.map(o => new StringSelectMenuOptionBuilder()
    .setLabel(o.label).setValue(o.value));
  const closeDaySelect = new StringSelectMenuBuilder()
    .setCustomId('setup:sch:add:r:close_day').setPlaceholder('Chọn ngày kết thúc...')
    .addOptions(closeDayOpts);

  const closeHourOpts = HOURS.map(h => new StringSelectMenuOptionBuilder()
    .setLabel(String(h).padStart(2, '0')).setValue(String(h)));
  const closeHourSelect = new StringSelectMenuBuilder()
    .setCustomId('setup:sch:add:r:close_hour').setPlaceholder('Chọn giờ kết thúc...')
    .addOptions(closeHourOpts);

  const closeMinOpts = MINUTES.map(m => new StringSelectMenuOptionBuilder()
    .setLabel(String(m).padStart(2, '0')).setValue(String(m)));
  const closeMinSelect = new StringSelectMenuBuilder()
    .setCustomId('setup:sch:add:r:close_min').setPlaceholder('Chọn phút kết thúc...')
...
module.exports = { renderAddViewStep1, renderAddViewStep2, DAYS, HOURS, MINUTES, CLOSE_DAY_OFFSETS };
