...
async function _moPhien(guild, lich) {
  ...
  const buttons = buildSessionActionRow(false); // [FIX] buildAttendanceButtons → buildSessionActionRow
  ...
  try {
    if (session.message_id) {
      ...
      if (msg) {
        ...
        const disabledBtns = buildSessionActionRow(true); // [FIX] buildAttendanceButtons → buildSessionActionRow
...
async function _khoiPhucCloseTimer(client, guild, danhSach) {
  ...
  log.info('SCHEDULER', guild.id, 'Restored close timer cho "%s": còn %s phút', lich.session_name, Math.round(msRemaining / 60000));
}
